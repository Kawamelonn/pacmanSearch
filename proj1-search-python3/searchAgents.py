...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        cornerStat = [item for item in state[1]]

        if (
            cornerStat[0] == "unvisited"
            or cornerStat[1] == "unvisited"
            or cornerStat[2] == "unvisited"
            or cornerStat[3] == "unvisited"
        ):
            return False
        return True
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            x, y = state[0][:]
            dx, dy = Actions.directionToVector(action)
            cornersStats = state[1][:]
            x2, y2 = int(x + dx), int(y + dy)

            if not self.walls[x2][y2]:
                nextPosition = (x2, y2)

                if nextPosition in self.corners:
                    cornersStats[self.corners.index(nextPosition)] = "visited"

                cost = 1
                nextNode = (nextPosition, cornersStats)
                successors.append((nextNode, action, cost))
...
def foodHeuristic(state, problem):
    ...
    curPoint = position
    while foodToEat:
        heuristic_cost, food = min(
            [(util.manhattanDistance(curPoint, food), food) for food in foodToEat]
        )
